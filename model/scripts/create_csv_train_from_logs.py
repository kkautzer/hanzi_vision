...
def create_csv():
    ...
    for filename in os.listdir("./model/logs"):
        # read log file and save each entry in data
        with open(f"./model/logs/{filename}", "r", encoding='utf-8') as log:
    ...
    with open("./model/exports/training_data.csv", "w", encoding='utf-8') as csv: 
        csv.writelines(f"{(",").join(entry)}\n" for entry in export_data)
        print("Log data successfully written to `./model/data/data_export.csv`")
        
if os.path.isfile("./model/exports/training_data.csv"):
